...
export const getDepartmentsForCollege = (college: string): string[] => {
  ...
  if (!key) return [];
  if (COLLEGE_DEPARTMENTS[key]) return COLLEGE_DEPARTMENTS[key];
  const normalized = key.toLowerCase();
  const fuzzyMatchKey = Object.keys(COLLEGE_DEPARTMENTS).find((name) => {
    const n = name.toLowerCase();
    return n === normalized || n.includes(normalized) || normalized.includes(n);
  });
  if (fuzzyMatchKey) return COLLEGE_DEPARTMENTS[fuzzyMatchKey];
  return ['Other'];
};
