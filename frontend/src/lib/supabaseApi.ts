...
import { getAuthEmailRedirectUrl, getSiteUrl } from './siteUrl';
...
const assertValidSchoolYear = (value: string): string => {
  const normalized = String(value || '').trim().toUpperCase();
  const match = normalized.match(SCHOOL_YEAR_PATTERN);
  if (!match) {
    throw new Error('Invalid School Year format. Use "SY 2025-2026".');
  }
  const firstYear = Number(match[1]);
  const secondYear = Number(match[2]);
  if (secondYear !== firstYear + 1) {
    throw new Error('Invalid School Year: second year must be exactly one year after the first year.');
  }
  return normalized;
};
...
export const profilesAPI = {
  ...
  update: async (id: string, updates: Partial<Profile>): Promise<Profile> => {
    ...
    if (!data) {
      throw new Error(
        'Profile update did not apply (no matching row or insufficient permission). If this is a new user, the email may already exist in Authentication — remove the old Auth user in Supabase Dashboard, then try again.'
      );
    }
    return withNormalizedRole(data);
  },
...
/**
 * Removes a user from Supabase Auth (and `profiles` via ON DELETE CASCADE when configured).
 * Returns true if the Edge Function removed the Auth user.
 * Returns false if the function is missing (404), unreachable (network/CORS), or the caller should
 * run `profilesAPI.delete` and clean up Auth manually / after deploy.
 */
async function invokeDeleteUserEdge(body: { userId: string } | { email: string }) {
  return supabase.functions.invoke<{ ok?: boolean; purged?: boolean }>('delete-user', { body });
}

/**
 * Best-effort: removes an Auth user that has no `profiles` row (same email), e.g. after profile-only delete.
 * No-op if the function is missing or unreachable.
 */
export async function purgeOrphanAuthUserByEmailViaEdgeFunction(email: string): Promise<void> {
  const normalized = String(email || '').trim().toLowerCase();
  if (!normalized) return;
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) return;

  let { error, response } = await invokeDeleteUserEdge({ email: normalized });
  if (!error) return;
  if (response?.status === 404 || !response) {
    await new Promise((r) => setTimeout(r, 400));
    ({ error, response } = await invokeDeleteUserEdge({ email: normalized }));
    if (!error) return;
  }
}

export async function deleteAuthUserViaEdgeFunction(userId: string): Promise<boolean> {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) throw new Error('Not authenticated');

  let { error, response } = await invokeDeleteUserEdge({ userId });

  if (!error) return true;
  if (response?.status === 404) return false;
  if (!response) {
    await new Promise((r) => setTimeout(r, 400));
    ({ error, response } = await invokeDeleteUserEdge({ userId }));
    if (!error) return true;
    if (response?.status === 404) return false;
    if (!response) {
      console.warn(
        '[deleteAuthUserViaEdgeFunction] delete-user Edge Function unreachable after retry. Deploy: supabase functions deploy delete-user (see frontend/supabase/functions/delete-user/index.ts).'
      );
      return false;
    }
  }
  ...
  throw new Error(text || `Could not delete auth user (${response.status})`);
}
...
export const registrationAPI = {
  ...
  signUp: async (payload: {
    ...
    const emailRedirect = payload.emailRedirectTo ?? getAuthEmailRedirectUrl();
    const { data, error } = await supabase.auth.signUp({
      email,
      ...
      options: {
        ...
        data: {
          ...
          faculty_department_id: payload.departmentId ?? null,
  ...
  resendConfirmation: async (email: string, emailRedirectTo?: string) => {
    const redirect = emailRedirectTo ?? getAuthEmailRedirectUrl();
...
export const collegesAPI = {
  ...
  update: async (
    ...
    if (!data) {
      throw new Error(
        'College update did not apply (no row returned). Check that your account can update colleges and that the college id is valid.'
      );
    }
    return data;
  },
...
export const departmentsAPI = {
  ...
  getByCollegeId: async (collegeId: string, activeOnly = false): Promise<Department[]> => {
    if (!collegeId) return [];
    let query = supabase
      .from('departments')
      .select('*')
  ...
  getActiveByCollegeName: async (collegeName: string): Promise<Department[]> => {
    const name = (collegeName || '').trim();
    if (!name) return [];
    ...
    return departmentsAPI.getByCollegeId(college.id, true);
  },

  create: async (payload: { college_id: string; name: string; is_active?: boolean }): Promise<Department> => {
  ...
  update: async (id: string, updates: Partial<Pick<Department, 'name' | 'is_active'>>): Promise<Department> => {
    const patch = { ...updates } as Partial<Pick<Department, 'name' | 'is_active'>>;
...
export const budgetsAPI = {
  ...
  createSession: async (budget: { academic_year: string; total_amount: number }): Promise<Budget> => {
    const validatedAcademicYear = assertValidSchoolYear(budget.academic_year);
    const latest = await budgetsAPI.getLatestSession();
    ...
    const { data, error } = await supabase
      .from('budgets')
      .insert({ ...budget, academic_year: validatedAcademicYear, total_amount: effectiveTotal, spent_amount: 0 })
      .select()
      .single();
    if (error) throw error;
    return data;
...
export const auditAPI = {
  ...
  /** Client-side audit row (RLS: actor_id must equal auth.uid() or null). */
  insert: async (payload: {
    ...
    if (error) console.warn('[auditAPI.insert]', error.message);
  },
};
