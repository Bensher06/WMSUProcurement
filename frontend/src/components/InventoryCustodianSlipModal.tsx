  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-3 sm:p-6 pt-8 sm:pt-12">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative z-10 w-full max-w-6xl max-h-[92vh] overflow-y-auto rounded-xl bg-white shadow-2xl border border-gray-200">
        ...
        <div className="px-4 py-6 sm:px-8 sm:pb-6">
          {!previewMode ? (
            ...
            <div className="space-y-5">
              <div className="rounded-xl border-2 border-red-900/15 bg-gradient-to-b from-white to-gray-50/90 p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 mb-4">
                  <div>
                    <p className="text-xs font-semibold text-red-900 uppercase tracking-wider">Property Management Office</p>
                    <h3 className="text-xl font-bold text-gray-900 mt-1">Inventory Custodian Slip</h3>
                    <p className="text-sm text-gray-600 mt-1">Item: <span className="font-medium text-gray-900">{request.item_name}</span></p>
                  </div>
                  <div className="text-right text-sm space-y-1">
                    <p className="text-gray-600"><span className="text-gray-500">ICS No:</span> {icsNo || '-'}</p>
                    <p className="text-gray-600"><span className="text-gray-500">Status:</span> {request.status}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                  <div className="rounded-lg border border-gray-200 bg-gray-50/80 p-4">
                    <p className="text-xs font-bold text-red-900 uppercase tracking-wide mb-2">Received by</p>
                    <p className="text-sm text-gray-900">{receivedByName || '-'}</p>
                    <p className="text-sm text-gray-600">{receivedByPosition || '-'}</p>
                    <p className="text-sm text-gray-600">{receivedByDate || '-'}</p>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-gray-50/80 p-4">
                    <p className="text-xs font-bold text-red-900 uppercase tracking-wide mb-2">Received from</p>
                    <p className="text-sm text-gray-900">{receivedFromName || '-'}</p>
                    <p className="text-sm text-gray-600">{receivedFromOffice || '-'}</p>
                    <p className="text-sm text-gray-600">{receivedFromDate || '-'}</p>
                  </div>
                </div>
              </div>
            </div>
