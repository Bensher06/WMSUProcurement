...
import { Check, Loader2, MessageSquareWarning, X, XCircle } from 'lucide-react';
...
export default function RequisitionViewModal({ request, onClose, onRecorded }: Props) {
  ...
  /** Dept head edits to Approved by / Issued by; cleared when switching requests. */
  const [collegeSigOverride, setCollegeSigOverride] = useState<{
  ...
  const showDeliveryForm =
    !!request && isDeptHead() && !!myCollege && ['Approved', 'Ordered'].includes(request.status);
      ...
      if (structuredParsed) {
        updates.description = serializeStructuredRequisition({
          ...
          signatories: {
            ...
            approvedBy: collegeSigOverride?.approvedBy ?? structuredParsed.signatories.approvedBy,
            issuedBy: collegeSigOverride?.issuedBy ?? structuredParsed.signatories.issuedBy,
          },
        });
      }
  ...
  const collegeSignatoriesEdit =
    structuredParsed && isDeptHead() && request.status === 'Pending'
      ? {
          approvedBy: collegeSigOverride?.approvedBy ?? structuredParsed.signatories.approvedBy,
          issuedBy: collegeSigOverride?.issuedBy ?? structuredParsed.signatories.issuedBy,
          onChange: setCollegeSigOverride,
        }
      : null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-3 sm:p-6 pt-8 sm:pt-12">
      ...
      >
        ...
        <div className="px-4 py-6 sm:px-8 sm:pb-4">
          {parsed && (
            <RequisitionDocumentView
              ...
              collegeSignatoriesEdit={collegeSignatoriesEdit}
            />
          )}

          {showDeliveryForm && (
            <div className="mt-8 rounded-xl border border-blue-200 bg-blue-50/60 p-4 space-y-3">
              <h3 className="text-sm font-semibold text-blue-950">Record delivery (logistics)</h3>
              <p className="text-xs text-blue-900/80">
                Enter actual quantity received. If less than ordered, provide mandatory remarks (e.g. partial shipment,
                stock-out).
              </p>
        ...
        {adjustEditMode && (
          <div className="sticky bottom-0 z-20 border-t border-amber-200 bg-amber-50/95 backdrop-blur-sm px-4 py-4 sm:px-8">
        ...
        {showApprovalBar && (
          <div className="sticky bottom-0 z-20 border-t border-gray-200 bg-gray-50/95 backdrop-blur-sm px-4 py-4 sm:px-8">
            <div className="flex flex-col gap-3 max-w-5xl mx-auto">
              <div>
                    ...
                    <>
                      ...
                      {budgetTypes.map((t) => (
                        <option key={t.id} value={t.id}>
                          {t.name}
                          {t.fund_code ? ` (${t.fund_code})` : ''} — ₱{Number(t.amount).toLocaleString()}
                        </option>
                      ))}
                    </>
                  )}
