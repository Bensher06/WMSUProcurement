...
export default function RequisitionViewModal({ request, onClose, onRecorded }: Props) {
  ...
  const onDownloadForm = () => {
    ...
    const lines = [
      ...
      `Requester: ${displayRequesterFullName(request)}`,
      `College: ${request.requester?.department || request.requester_college || '—'}`,
      `Department: ${displayRequesterFacultyDepartment(request) || '—'}`,
      '',
      '--- DESCRIPTION ---',
              ...
              {commentsLoading ? (
                ...
                comments.map((c) => (
                  <div key={c.id} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm">
                    <p className="font-medium text-gray-900">
                      {c.author?.full_name || c.author?.email || (c.author_id ? 'User' : 'Former user')}
                    </p>
