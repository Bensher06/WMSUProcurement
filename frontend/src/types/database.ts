...
export type ProfileStatus = 'Pending' | 'Approved' | 'Declined' | 'Suspended';
    ...
    Tables: {
      profiles: {
        Row: {
          ...
          faculty_department_id: string | null;
      ...
      requests: {
        Row: {
          ...
          /** Copied from requester profile; kept when the profile is removed. */
          requester_college: string | null;
          /** Copied from requester profile; kept when the profile is removed. */
          requester_faculty_department: string | null;
...
/** Faculty department when the joined profile is missing (account removed; row kept for history). */
export function displayRequesterFacultyDepartment(r: RequestWithRelations): string {
  return (r.requester?.faculty_department || r.requester_faculty_department || '').trim();
}

/** Requester display name; uses frozen faculty/college context only for department text elsewhere. */
export function displayRequesterFullName(r: RequestWithRelations): string {
  const n = r.requester?.full_name?.trim();
  if (n) return n;
  return r.requester_id ? '—' : 'Former user';
}
