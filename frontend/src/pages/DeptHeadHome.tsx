...
export default function DeptHeadHome() {
  ...
  const notifications = useMemo(
    () => requests.filter((r) => ['Negotiating', 'Rejected'].includes(r.status)).length,
    [requests]
  );
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Link to="/dept-head/request-history" className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
              <div className="flex items-center justify-between">
            ...
            <Link to="/dept-head/request-history" className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
              ...
              <p className="text-xs text-gray-500 mt-1">Draft and pending requests</p>
            </Link>
          </div>
