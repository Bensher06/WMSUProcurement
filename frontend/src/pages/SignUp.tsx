...
import { collegesAPI, departmentsAPI, registrationAPI } from '../lib/supabaseApi';
...
export default function SignUp() {
  ...
  useEffect(() => {
    ...
    const loadDepartments = async () => {
      const collegeName = form.college.trim();
      if (!collegeName) {
        setDepartments([]);
        return;
      }
      ...
      try {
        const rows = await departmentsAPI.getActiveByCollegeName(collegeName);
        if (!cancelled) setDepartments(rows);
      } catch (e: any) {
        if (!cancelled) {
          setDepartments([]);
          setError(e?.message || 'Could not load departments for the selected college.');
        }
      } finally {
  ...
  const selectedDepartment = useMemo(
    () => departments.find((d) => d.name === form.department) ?? null,
  ...
  const handleResend = async () => {
    ...
    setResendError('');
    try {
      await registrationAPI.resendConfirmation(submittedEmail, getAuthEmailRedirectUrl());
      setResendState('sent');
    } catch (err: any) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!form.college || loadingDepartments || departments.length === 0}
                  required
                >
                  <option value="">
                    {!form.college
                ...
                {!!form.college && !loadingDepartments && departments.length === 0 && (
                  <p className="text-xs text-amber-700 mt-1">
                    No departments yet for this college. Please ask your College Admin to create departments first.
                  </p>
                )}
              </div>
            </div>
