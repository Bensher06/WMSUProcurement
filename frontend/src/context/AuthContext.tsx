...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const readCachedProfile = (userId: string): Profile | null => {
    try {
      ...
      return {
        ...
        first_name: (parsed as any).first_name,
        middle_initial: (parsed as any).middle_initial ?? null,
        family_name: (parsed as any).family_name,
        email: String(parsed.email ?? ''),
        role: normalizeUserRole(parsed.role as any),
        status: ((parsed as any).status as Profile['status']) || 'Approved',
        department: (parsed.department ?? null) as any,
        faculty_department: (parsed.faculty_department ?? null) as any,
        faculty_department_id: (parsed as any).faculty_department_id ?? null,
  ...
  const fetchProfile = async (userId: string) => {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select(
          'id,role,status,full_name,first_name,middle_initial,family_name,email,department,faculty_department,faculty_department_id,approved_budget,created_at,updated_at'
        )
        .eq('id', userId)
        .maybeSingle();
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (event === 'INITIAL_SESSION' || event === 'TOKEN_REFRESHED') {
          ...
          setUser(session?.user ?? null);
          setLoading(false);
          // Email confirmation / magic link can populate the session after the first
          // `getSession()` returned null; ensure profile loads for that user.
          if (event === 'INITIAL_SESSION' && session?.user?.id) {
            const uid = session.user.id;
            if (uid !== lastProfileUserIdRef.current) {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      if (uid) {
        const { data: gateProfile, error: gateErr } = await supabase
          .from('profiles')
          .select('status, role')
          .eq('id', uid)
          .maybeSingle();
        if (!gateErr && gateProfile) {
          const st = (gateProfile.status as string) || 'Approved';
          if (st === 'Suspended' || st === 'Declined') {
            setUser(null);
            setProfile(null);
            setSession(null);
            setProfileLoading(false);
            await supabase.auth.signOut();
            throw new Error(
              'This account is no longer active. Contact your administrator if you need access.'
            );
          }
          if (st === 'Pending') {
            setUser(null);
            ...
            await supabase.auth.signOut();
            throw new Error(
              'Your account is pending approval from your College Admin or WMSU administrator.'
            );
          }
        }
      }
